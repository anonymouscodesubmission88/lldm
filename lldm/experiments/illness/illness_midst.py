...
if __name__ == '__main__':
    ...
    description = f"{'Residual_' if residual_dynamics else ''}" \
                  f"{observable_dim}K_" \
                  f"{window_size * trajectory_length}W_" \
                  f"{prediction_horizon}H"

    experiment_name = f"Illness_MIDST_{description}_{date}"
    ...
    train_ratio = 0.7
    val_ratio = 0.1
    file_path = os.path.join(DATA_DIR, 'illness', 'national_illness.csv')
    standard_scalar_scale = True
    min_max_scale = False
    use_windows_of_states = True
    train_ds = TSDataset(
        filepath=file_path,
        ...
        use_windows_of_states=use_windows_of_states,
    )
    val_ds = TSDataset(
        filepath=file_path,
        window_size=window_size,
    ...
    last_window_pred_only = True
    loss_trajectory_length = trajectory_length * window_size
    loss_fn = ModuleLoss(
        model=torch.nn.MSELoss(),
        last_window_pred_only=last_window_pred_only,
        trajectory_length=loss_trajectory_length,
    )
    loss_fn_params = {
        'loss_functions': 'MSE',
    }
